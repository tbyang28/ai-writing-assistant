...
<template>
  <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
    ...
    <div class="flex-1 overflow-auto">
      <div class="max-w-6xl mx-auto px-6 py-6">
        <!-- Loading -->
        <div v-if="bookStore.isLoading" class="py-16 text-center text-gray-400">
        ...
        <div v-else-if="bookStore.books.length > 0" class="grid gap-5 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          <div v-for="book in bookStore.books" :key="book.id" @click="openBook(book)" class="group cursor-pointer">
            <div class="aspect-[3/4] rounded-lg overflow-hidden relative shadow-sm bg-gradient-to-br from-brand-50 to-blue-100 flex items-center justify-center border border-gray-200">
              <span class="text-gray-300 text-5xl font-serif">{{ book.title.charAt(0) }}</span>
        ...
        <div v-else class="text-center py-20">
          ...
          <h2 class="text-lg font-semibold text-gray-900 mb-2">暂无作品</h2>
          <p class="text-sm text-gray-500 mb-6">点击"新建作品"开始你的第一部作品</p>
    ...
    <div v-if="showNewBookModal" class="modal-overlay" @click.self="showNewBookModal = false">
      <div class="modal-content">
        <h3 class="text-lg font-semibold text-gray-900 mb-5">新建作品</h3>
        <div class="space-y-4">
          <div>
          ...
          <div v-if="createError" class="text-sm text-red-500 bg-red-50 px-3 py-2 rounded-lg">{{ createError }}</div>
        </div>
